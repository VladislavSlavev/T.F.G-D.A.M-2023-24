...
import com.campusfp.remarket.databinding.ItemComentarioBinding
...
class AdaptadorComentario : RecyclerView.Adapter<AdaptadorComentario.HolderComentario> {
    ...
    override fun onBindViewHolder(holder: HolderComentario, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            ...
            if (firebaseAuth.uid.equals(modelo.uid)) {
    ...
    // Cuadro de dialogo para eliminar un comentario
                ...
                if (firebaseAuth.uid.equals(uidUsuario)){
                    ...
                    ref.child(uidVendedor).child("Comentarios").child(idComentario)
                        ...
                        .addOnSuccessListener {
                            Toast.makeText(context, "El comentario se ha eliminado", Toast.LENGTH_SHORT).show()
                        }
                        .addOnFailureListener { e->
                            Toast.makeText(context, "No se puede eliminar el comentario debido a ${e.message}", Toast.LENGTH_SHORT).show()
                        }
                }
            }
    ...
    // Cargar los datos del usuario que ha hecho el comentario
        ...
        ref.child(uid)
            .addListenerForSingleValueEvent(object : ValueEventListener{
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    try {
                        Glide.with(context).load(imagen).placeholder(R.drawable.img_perfil).into(holder.IvImagen)
    ...
    inner class HolderComentario (itemView : View) : RecyclerView.ViewHolder(itemView){
        val IvImagen = binding.IvImgPerfil
        val Tv_nombres = binding.TvNombresCom
        val Tv_fecha = binding.TvFechaCom
